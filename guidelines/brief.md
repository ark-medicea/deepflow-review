...
├── components/
│   ├── shell/                    # Sidebar, ChatPanel, TopBar, HubBar (HubBar/, HubBarCollapsed, HubBarExpanded, HubBarStats, HubBarTeam, HubBarResources, useHubBarData)
...
| `ProjectCard` | Progress bar, status counts, avatar stack. Click → navigate to project |
...
| **Hub Bar (expanded)** | "Share progress" → generates a snapshot URL | Progress dashboard: stats, graph thumbnail, due dates. Auto-updates |
